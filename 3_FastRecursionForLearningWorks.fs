...
let chooseDirection (ghost:Ghost) =
  ...
  let xs =
    directions
    |> Array.map fst
    //This filters out directions towards pacman
...
*)
  ...
  let rec update () =
    ...
    elif Keyboard.isPressed 38 (*up*) then
         window.setTimeout(update, 150) |> ignore
    else
        //window.setTimeout(update, 10) |> ignore
        update () |> ignore
...
let rec game () =
  ...
  let levelCompleted () =
    ...
    window.setTimeout(game, 1) |> ignore
    //game()
  ...
  let start () =   
    if gamesPlayed > 20 then
            ...
            window.setTimeout(endSession,5000) |> ignore
    else playLevel (levelCompleted, gameOver)
...
canvas.addEventListener("click", fun _ -> game ())