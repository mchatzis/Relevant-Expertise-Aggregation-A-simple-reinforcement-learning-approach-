  ...
  *)
  ...
  let moveGhosts () =
    ghosts |> Array.iter (fun ghost ->
                                let dir' = match chooseDirection ghost with
                                           |(-1,0) -> if canGoLeft(ghost.X,ghost.Y) then (-1,0) else (0,0)
                                           |(0,1) -> if canGoDown(ghost.X,ghost.Y) then (0,1) else (0,0)
                                           |(1,0) -> if canGoRight(ghost.X,ghost.Y) then (1,0) else (0,0)
                                           |(0,-1) -> if canGoUp(ghost.X,ghost.Y) then (0,-1) else (0,0)
                                           |_ -> (0,0)
                           
                                ghost.Move(dir')
                                )

  ///Calculates distance of two coordinates
  ...
  //let findClosestGhostDistance (dx,dy) (ghosts:Ghost [])=
  //   ghosts
  //   |> Array.map (fun a -> (a.X,a.Y))
  //   |> Array.map (fun (a,b) -> distance (a,b) (!x + dx,!y + dy))
  //   |> Array.min 

  let isPillOrGate (vx,vy) =
        let mutable pillExists = false
        let mutable gateExists = false
        [1..10]
        |> List.iter (fun i -> 
                               let x',y' = (!x + i * vx),(!y + i * vy)
                               ...
                               if pills.[ty].[tx] = '=' 
                                   then 
                                       ...
                                       ()
                               ())
        pillExists,gateExists

  //let existsGateAhead (vx,vy) =
  //     let mutable gateExists = false
  //     [1..10]
  //     |> List.iter (fun i -> let x',y' = (!x + i * vx),(!y + i * vy)
  //                            let tx = int (floor(float ((x'+ 6)/8)))
  //                            let ty = int (floor(float ((y'+ 6)/8)))
  //                            if pills.[ty].[tx] = '=' 
  //                                 then 
  ...
  let filterPillDirections avDirections = 
        avDirections
        |> Array.filter (fun a -> match (isPillOrGate a) with
                                  |(true,_) -> true
                                  |(false,_) -> false)
  
  ///// True when there is a ghost in the line of sight of the pacman
  //let existsGhostAhead (ghosts:Ghost []) =
  
  //      match !v with
  //      |(dx,0) -> let ghostCandidates =
  //                      ghosts
  //                      |> Array.filter (fun a -> (a.Y = !y) )
  //                      |> Array.filter (fun a -> sign(a.X - !x) = dx)
  //                      |> Array.sortBy (fun a -> distance (!x,!y) (a.X,a.Y))
  //                 if Array.isEmpty ghostCandidates then false
  //                 elif (distance (!x,!y) (ghostCandidates.[0].X,ghostCandidates.[0].Y) > 30) then false
  //                 else 
  //                      true
  //      |(0,dy) -> let ghostCandidates =
  //                      ghosts
  //                      |> Array.filter (fun a -> (a.X = !x) )
  //                      |> Array.filter (fun a -> sign(a.Y - !y) = dy)
  //                      |> Array.sortBy (fun a -> distance (!x,!y) (a.X,a.Y))
  //                 if Array.isEmpty ghostCandidates then false
  //                 elif (distance (!x,!y) (ghostCandidates.[0].X,ghostCandidates.[0].Y) > 30) then false
  //                 else 
  ...
  ///Looking n steps ahead and assuming ghosts keep same direction
  ///,if distance of ghosts with pacman becomes smaller than Zone
  ///return true (there is a collision danger).
  ///Else return false
  let collisionDanger ()=
        let dangerRadius = 30;
        let deadlyZone = 9;
        let mutable isGhostDeadly = false;
        let stepsAhead = 10;


        let dangerousGhosts =
                ghosts
                |> Array.filter (fun a -> distance (!x,!y) (a.X,a.Y) < dangerRadius)
                |> Array.iter (fun a -> [1..2..stepsAhead]
                                        |> List.iter (fun step -> 
                                                        let (vxGhost,vyGhost) = a.V;
                                                        let (xghost',yghost') = vxGhost*step + a.X,vyGhost * step + a.Y;
                                                        let (vxPac,vyPac) = !v;
                                                        let (xpac',ypac') = vxPac*step + !x,vyPac * step + !y;
                                                        if (distance (xpac',ypac') (xghost',yghost')) < deadlyZone
                                                                then isGhostDeadly <- true
                                                        else ()
                                                      )
                               )
        isGhostDeadly
  ...
  let movePacman () =
    ...
    /// No walls or gates allowed
    let noWallDirections =
       [| canGoUp (!x,!y), (0,-1)
          canGoDown (!x,!y), (0,1)          
          canGoLeft (!x,!y), (-1,0)          
          canGoRight (!x,!y), (1,0) |]
       |> Array.filter fst //directions with no wall forward or gate
       |> Array.map snd //select just direction part of tuple
       |> Array.filter (fun a -> match isPillOrGate a with
                                 |(_,true) -> false
                                 |(_,false) -> true)
    ...
    let availableDirections =
       noWallDirections
       |> Array.filter (fun (a,b) -> match backwardDir with
                                     |(c,d) -> not (a = c && b = d)) //backwards not allowed
    ...
    let randomNum = System.Random().NextDouble()
    let checkDanger =
        //match existsGhostAhead ghosts with
        //|true -> v := backwardDir
        //|false ->  
        match collisionDanger () with
                    | true -> 
                            let removeForwDir =
                                noWallDirections
                                |> Array.filter(fun a -> a = !v |> not)
                            let i = randomNum * float removeForwDir.Length
                            v := removeForwDir.[int (floor i)]
                    | false ->
                                match filterPillDirections availableDirections with
                                |[||] -> let i = randomNum * float availableDirections.Length
                                         v := availableDirections.[int (floor i)]
                                        
                                |a ->
                                            let i = randomNum * float a.Length
                                            v := a.[int (floor i)]
  ...
  let eatPills () =
    ...
    if c = 'o' then
      ...
      ////I disabled power mode for simplicity!
      //pills.[ty].[tx] <- ' '
      //clearCell background (tx,ty)
      //score := !score + 10
      //decr dotsLeft
  ...
  let touchingGhosts () =
    let allowedDistance = 7;
    let px, py = !x, !y
    ghosts |> Array.filter (fun ghost ->
      let x,y = ghost.X, ghost.Y
      ((px >= x && px < x + allowedDistance) ||
       (x < px + allowedDistance && x >= px)) &&
      ((py >= y && py < y + allowedDistance) ||
       (y < py + allowedDistance && y >= py)) )
      ...
      if !powerCountdown > 0 then
        ...
        touched |> Array.iter (fun ghost ->
          if not ghost.IsReturning then
            ...
            bonuses := (100, (ghost.X, ghost.Y)) :: !bonuses
            bonus :=  min 3 (!bonus + 1) )
      else
...
//    renderBonus ()
  ...
  //let mutable pacManWon = false;
  ////counting update stack calls
  //let mutable callStack = 0;
  //let mutable stackFull = false;

  while ((!dotsLeft = 0) || (!energy<=0)) |> not do
    logic()
  
  if !dotsLeft = 0 then true
  else false

    //callStack <- callStack + 1;
    //logic ()
    //render ()
    //if callStack > 3000 then stackFull <- true;
    //elif !dotsLeft = 0 then pacManWon <- true;
    //elif !energy <= 0 then ()
    //else update ()
    //update ()
...
let rec game () =
  ...
  //Reset keyboard and initialize canvas
  Keyboard.reset()
  ...
  //Define games to play and count wins
  ...
  let rec start () =   
    if gamesPlayed > 9
        then
            let finalCount = "Pacman won " + string(totalWins) + " out of " + string(gamesPlayed) + " games!";
            context.drawImage(!^ background, 0., 0.)
            context.fillStyle <- !^ "white"
            context.font <- "bold 8px";
            drawText(finalCount,63.,96.)
            window.setTimeout(game,1000) |> ignore
    else match playLevel () with
         | true ->     gamesPlayed <- gamesPlayed + 1
                       totalWins <- totalWins + 1
                       context.drawImage(!^ background, 0., 0.)
                       context.fillStyle <- !^ "white"
                       context.font <- "bold 8px";
                       let text = string(gamesPlayed);
                       drawText("COMPLETED",96.,96.)
                       drawText("Games Played: " + text,85.,123.)
                       window.setTimeout(start,1)|> ignore
                       //start()
         | false ->    gamesPlayed <- gamesPlayed + 1
                       context.drawImage(!^ background, 0., 0.)
                       context.fillStyle <- !^ "white"
                       context.font <- "bold 8px";
                       let text = string(gamesPlayed);
