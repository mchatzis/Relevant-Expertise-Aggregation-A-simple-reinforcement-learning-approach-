...
let playLevel (onLevelCompleted, onGameOver) =
  ...
  *)
  ...
  let filterPillDirections avDirections = 
        avDirections
        |> Array.filter (fun a -> existsPillAhead a)
  ...
  ///,if distance of ghosts with pacman becomes smaller than deadlyZone
  ...
  let collisionDanger ()=
        ...
        let dangerousGhosts =
                ghosts
                |> Array.filter (fun a -> distance (!x,!y) (a.X,a.Y) < dangerRange)
                |> Array.iter (fun a -> [1..stepsAhead]
                                        |> List.iter (fun step -> 
  ...
  let movePacman () =
    ...
    ///Two types of non-allowed directions: 1) walls 2) backwards
    let availableDirections =
       ...
       |> Array.filter (fun a -> (existsGateAhead a) |> not)
...
*)
  ...
  let rec update () =
    ...
    if !dotsLeft = 0 then onLevelCompleted()
         ...
         window.setTimeout(update, 30) |> ignore
    else
        window.setTimeout(update, 1) |> ignore
        //update () |> ignore
