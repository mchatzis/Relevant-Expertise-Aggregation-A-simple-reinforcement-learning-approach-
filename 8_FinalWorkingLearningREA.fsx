    ...
    *)
    ...
    type Ghost(image: HTMLImageElement,x,y,v,id) =
      ...
      member __.UpdPerf(victorious,ruleIndex) =
        let (a,b) = performance'.[ruleIndex]
        if victorious then
            performance'.[ruleIndex] <- (a + 1,b+1)
        else 
            performance'.[ruleIndex] <- (a,b+1)
...
let allGhosts =
            ...
            |> List.toArray 
            |> Array.map (fun index -> 
                        createGhosts.[index].Identify(index) |> ignore
                        createGhosts.[index])
...
let createRuleset () =
    ...
    /// No rules, pure chaos
    let rule1 (arg1 : (int*int) []) (_:Ghost) : (int*int) [] =
        allDirections

    /// Filter out the directions of walls
    let rule2 (arg1 : (int*int) []) (ghost:Ghost): (int*int) [] =
    ...
    let rule3 (arg1 : (int*int) []) (ghost:Ghost) : (int*int) [] =
        let (dx,dy) = ghost.V
        arg1
        |> Array.filter (not << (isBackwards (dx,dy)))
  
    ///ALlowed directions are only left and right
    let rule4 (_ : (int*int) []) (_:Ghost) : (int*int) [] = 
      [|
        (-1,0) // left
        (1,0) //right
      |]  
    
    ///Rule2 and Rule 3 combined
    let rule5 (arg1 : (int*int) []) (ghost:Ghost): (int*int) [] = 
        let (x,y) = (ghost.X,ghost.Y)
        let (dx,dy) = ghost.V
        arg1
        |> Array.filter (fun dir -> 
...
let chooseDirection (ghost:Ghost) (rule:Ghost -> (int*int)[])=
  let x,y = ghost.X, ghost.Y
  let dx,dy = ghost.V
...
let playLevel () =
  ...
  *)
  ...
  let mutable ghostPool = allGhosts
  let ghosts = 
        [|1..4|]
  ...
  *)
  ...
  let mutable opinions = [|0;0;0;0;0;|]
  let formOpinions = 
  ...
  let maxRules =
            formOpinions
            |> Array.filter (fun (rule,opin) -> 
                                                opin = bestOpinion)
            |>Array.map fst
  
  let randomNum = System.Random().NextDouble()
  let k = randomNum * float maxRules.Length
  let ruleTuple = maxRules.[int (floor k)]
  let (rule',rIndex) = ruleTuple
  let rule = rule' allDirections

  let moveGhosts () =
    ghosts |> Array.iter (fun ghost -> 
                                let dir' = match chooseDirection ghost rule with
...
*)
  ...
  if !dotsLeft = 0 then putBack false
                        true
  else 
        putBack true
        false
    ...
    if gamesPlayed > 199
        then
         ...
         | true ->     
                       ...
                       //start()
         | false ->    wins <- Array.append wins [|1|]
                       gamesPlayed <- gamesPlayed + 1
                       context.drawImage(!^ background, 0., 0.)
                       context.fillStyle <- !^ "white"
                       context.font <- "bold 8px";
